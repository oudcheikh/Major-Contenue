...
# ===========================================================================
_villes = [
    # (x, y, nom, dx, dy, anchor) — positions issues des vraies coordonnées
    (95, 242, "انواكشوط ★", 8, 0, "start"),
    (73, 178, "انواذيبو", 8, -10, "start"),
    (176, 136, "ازويرات", 9, 4, "start"),
    (162, 187, "أطار", -8, -4, "end"),
    (178, 188, "شنقيط", 2, 15, "middle"),
    (195, 177, "ودان", 8, -3, "start"),
    (200, 232, "تجكجة", 9, 4, "start"),
    (200, 276, "كيفة", 2, -9, "middle"),
    (296, 276, "النعمة", 2, 14, "middle"),
    (99, 279, "روصو", -7, -5, "end"),
    (182, 310, "سيلبابي", 2, -12, "middle"),
    (301, 261, "ولاتة", 2, -10, "middle"),
    (242, 275, "عيون العتروس", 12, 14, "start"),
]

_villes_svg = ""
for _x, _y, _nom, _dx, _dy, _anc in _villes:
    _star = "★" in _nom
    _r = 5 if _star else 3.6
    _fill = ROUGE if _star else TRAIT
    _villes_svg += (
        f'<circle cx="{_x}" cy="{_y}" r="{_r}" fill="{_fill}" '
        f'stroke="#ffffff" stroke-width="1.2"/>'
        f'<text x="{_x + _dx}" y="{_y + _dy + 4}" font-size="11" '
        f'font-weight="600" fill="{TRAIT}" text-anchor="{_anc}">{_nom}</text>'
    )

CARTE_MAURITANIE = (
    _svg_open()
    + f'''
  <rect x="1" y="1" width="418" height="358" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>
  <path d="{_OCEAN_POLY}" fill="{OCEAN}" opacity="0.8"/>
  <path d="{_MR}" fill="{SABLE}" stroke="{TRAIT}" stroke-width="2"/>
  <path d="{_RIVER}" fill="none" stroke="{BLEU}" stroke-width="3.2" stroke-linecap="round"/>
  <!-- طريق الأمل : انواكشوط - بوتلميت - ألاك - كيفة - عيون العتروس - النعمة -->
  ...
  <text x="250" y="253" font-size="11" font-weight="700" fill="{AMBRE}" text-anchor="middle">طريق الأمل</text>
  <!-- الجيران -->
  <text x="112" y="122" font-size="12" font-weight="700" fill="#8a6d3b" text-anchor="middle">الصحراء الغربية</text>
  <text x="352" y="48" font-size="13" font-weight="700" fill="#8a6d3b">الجزائر</text>
  <text x="382" y="200" font-size="13" font-weight="700" fill="#8a6d3b" text-anchor="middle">مالي</text>
  <text x="250" y="342" font-size="13" font-weight="700" fill="#8a6d3b" text-anchor="middle">مالي</text>
  <text x="118" y="330" font-size="13" font-weight="700" fill="#8a6d3b" text-anchor="middle">السنغال</text>
  <text x="30" y="250" font-size="13" font-weight="700" fill="{BLEU}"
        transform="rotate(-90 30 250)" text-anchor="middle">المحيط الأطلسي</text>
  <text x="112" y="300" font-size="11" font-weight="600" fill="{BLEU}">نهر السنغال</text>
  <!-- المدن -->
  {_villes_svg}
  <!-- وردة الاتجاهات المصغرة -->
  <g stroke="{TRAIT}" stroke-width="1.6">
    <line x1="45" y1="52" x2="45" y2="88"/>
    <line x1="27" y1="70" x2="63" y2="70"/>
    <path d="M 45,48 L 41,58 L 49,58 Z" fill="{ROUGE}" stroke="none"/>
  </g>
  <text x="45" y="44" font-size="11" font-weight="700" fill="{ROUGE}" text-anchor="middle">شمال</text>
  <text x="45" y="101" font-size="11" fill="{TRAIT}" text-anchor="middle">جنوب</text>
  <text x="68" y="74" font-size="11" fill="{TRAIT}" text-anchor="start">شرق</text>
  <text x="22" y="74" font-size="11" fill="{TRAIT}" text-anchor="end">غرب</text>
  <!-- مقياس -->
  <g stroke="{TRAIT}" stroke-width="1.6">
    <line x1="332" y1="338" x2="380" y2="338"/>
    <line x1="332" y1="333" x2="332" y2="343"/>
    <line x1="380" y1="333" x2="380" y2="343"/>
  </g>
  <text x="356" y="330" font-size="11" fill="{TRAIT}" text-anchor="middle">200 كم</text>
</svg>'''
)

# ===========================================================================
# 2. CARTE_VIERGE — fond de carte à compléter (exercice du concours)
# ===========================================================================
_vierge_spots = [
    # (x, y, lx1, ly, llen)  cercle vide + ligne pointillée pour écrire
    (95, 242, 104, 246, 46),
    (73, 178, 82, 170, 46),
    (176, 136, 186, 140, 46),
    (162, 187, 145, 201, 46),   # ligne sous le point (zone dense Adrar)
    (200, 232, 210, 236, 46),
    (200, 276, 210, 268, 46),
    (296, 276, 306, 280, 46),
    (99, 279, 58, 267, 40),
...
_vierge_svg = ""
for _x, _y, _lx, _ly, _ll in _vierge_spots:
    _vierge_svg += (
        f'<circle cx="{_x}" cy="{_y}" r="5" fill="#ffffff" stroke="{TRAIT}" stroke-width="1.6"/>'
...
CARTE_VIERGE = (
    _svg_open()
    + f'''
  <rect x="1" y="1" width="418" height="358" rx="10" fill="#ffffff" stroke="{TRAIT}" stroke-width="1.2"/>
  <path d="{_OCEAN_POLY}" fill="{OCEAN}" opacity="0.55"/>
  <path d="{_MR}" fill="#fefcf6" stroke="{TRAIT}" stroke-width="2"/>
  <path d="{_RIVER}" fill="none" stroke="{BLEU}" stroke-width="3" stroke-linecap="round"/>
  <text x="30" y="250" font-size="13" font-weight="700" fill="{BLEU}"
        transform="rotate(-90 30 250)" text-anchor="middle">المحيط الأطلسي</text>
  {_vierge_svg}
  <text x="210" y="30" font-size="14" font-weight="700" fill="{TRAIT}" text-anchor="middle">أَكْمِلِ الخريطة: اكتب أسماء المدن والجيران ولوّن</text>
  <!-- lignes vides pour les pays voisins -->
  <line x1="88" y1="118" x2="152" y2="118" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3 3"/>
  <line x1="300" y1="55" x2="360" y2="55" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3 3"/>
  <line x1="358" y1="200" x2="408" y2="200" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3 3"/>
  <line x1="88" y1="332" x2="145" y2="332" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3 3"/>
</svg>'''
)

# ===========================================================================
# 3. CARTE_RESSOURCES — ressources économiques
# ===========================================================================
CARTE_RESSOURCES = (
    _svg_open()
    + f'''
  ...
  <path d="{_MR}" fill="{SABLE}" stroke="{TRAIT}" stroke-width="2"/>
  <path d="{_RIVER}" fill="none" stroke="{BLEU}" stroke-width="3" stroke-linecap="round"/>
  <text x="210" y="24" font-size="14" font-weight="700" fill="{TRAIT}" text-anchor="middle">ثروات بلادي</text>
  <!-- حديد ازويرات -->
  <text x="182" y="146" font-size="16" text-anchor="middle">⛏️</text>
  <text x="198" y="150" font-size="11" font-weight="700" fill="{TRAIT}" text-anchor="start">حديد</text>
  <!-- ملح سبخة إيجل -->
  <text x="198" y="114" font-size="15" text-anchor="middle">🧂</text>
  <text x="214" y="118" font-size="11" font-weight="700" fill="{TRAIT}" text-anchor="start">ملح</text>
  <!-- صيد على الساحل -->
  <text x="60" y="212" font-size="16" text-anchor="middle">🐟</text>
  <text x="60" y="228" font-size="11" font-weight="700" fill="{BLEU}" text-anchor="middle">صيد</text>
  <!-- نفط في البحر -->
  <text x="58" y="268" font-size="15" text-anchor="middle">🛢️</text>
  <text x="58" y="284" font-size="11" font-weight="700" fill="{TRAIT}" text-anchor="middle">نفط</text>
  <!-- زراعة وادي النهر -->
  <text x="132" y="272" font-size="15" text-anchor="middle">🌾</text>
  <text x="144" y="270" font-size="11" font-weight="700" fill="{VERT}" text-anchor="start">زراعة</text>
  <!-- رعي الوسط والشرق -->
  <text x="255" y="222" font-size="16" text-anchor="middle">🐪</text>
  <text x="273" y="226" font-size="11" font-weight="700" fill="{AMBRE}" text-anchor="start">رعي</text>
  <!-- légende (en bas à gauche, sur l'océan) -->
  <rect x="8" y="298" width="118" height="56" rx="8" fill="#ffffff" stroke="{TRAIT}" stroke-width="1.2"/>
  <text x="118" y="314" font-size="11" font-weight="700" fill="{TRAIT}" text-anchor="start">⛏️ معادن   🐟 صيد</text>
  <text x="118" y="331" font-size="11" font-weight="700" fill="{TRAIT}" text-anchor="start">🌾 زراعة   🐪 رعي</text>
  <text x="118" y="348" font-size="11" font-weight="700" fill="{TRAIT}" text-anchor="start">🛢️ نفط   🧂 ملح</text>
</svg>'''
)

# ===========================================================================
# 4. CARTE_EMPIRES — Ghana et Almoravides en Afrique de l'Ouest
# ===========================================================================
CARTE_EMPIRES = (
    _svg_open("0 0 420 380")
    + f'''
  <rect x="1" y="1" width="418" height="378" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>
  <rect x="8" y="46" width="404" height="326" fill="{OCEAN}" opacity="0.45"/>
  <!-- الأندلس -->
  <path d="M 150,12 L 280,12 L 268,40 L 196,46 L 156,36 Z" fill="{SABLE}" stroke="{TRAIT}" stroke-width="1.6"/>
  <text x="215" y="32" font-size="13" font-weight="700" fill="{TRAIT}" text-anchor="middle">الأندلس</text>
  <!-- إفريقيا -->
  <path d="M 150,58 L 128,92 L 104,145 L 92,205 L 90,255 L 100,305 L 130,335 L 190,352 L 260,362 L 412,362 L 412,58 Z"
        fill="{SABLE}" stroke="{TRAIT}" stroke-width="2"/>
  <!-- مملكة المرابطين -->
  <ellipse cx="150" cy="195" rx="72" ry="112" fill="{VERT}" opacity="0.22" stroke="{VERT}" stroke-width="1.6"/>
  <text x="175" y="222" font-size="14" font-weight="700" fill="#1c6b40" text-anchor="middle">دولة المرابطين</text>
  <circle cx="138" cy="98" r="4" fill="{TRAIT}"/>
  <text x="150" y="102" font-size="11" font-weight="600" fill="{TRAIT}" text-anchor="start">مراكش</text>
  <!-- سهم المرابطين نحو الأندلس -->
  <path d="M 175,135 L 205,60" fill="none" stroke="{VERT}" stroke-width="2.6" marker-end="url(#arrE)"/>
  <path d="M 118,268 L 133,172" fill="none" stroke="{VERT}" stroke-width="2.6" stroke-dasharray="6 4" marker-end="url(#arrE)"/>
  <!-- إمبراطورية غانا -->
  <ellipse cx="258" cy="312" rx="74" ry="42" fill="{AMBRE}" opacity="0.28" stroke="{AMBRE}" stroke-width="1.6"/>
  <text x="258" y="305" font-size="14" font-weight="700" fill="#9a5b06" text-anchor="middle">إمبراطورية غانا</text>
  <circle cx="268" cy="327" r="4" fill="{ROUGE}"/>
  <text x="268" y="345" font-size="11" font-weight="700" fill="{ROUGE}" text-anchor="middle">كومبي صالح</text>
  <!-- الأنهار -->
  <path d="M 95,300 C 140,315 175,318 212,320" fill="none" stroke="{BLEU}" stroke-width="2.6"/>
  <text x="150" y="334" font-size="11" font-weight="600" fill="{BLEU}">نهر السنغال</text>
  <text x="45" y="220" font-size="12" font-weight="700" fill="{BLEU}" transform="rotate(-90 45 220)" text-anchor="middle">المحيط الأطلسي</text>
  <text x="330" y="120" font-size="12" font-weight="700" fill="#8a6d3b" text-anchor="middle">الصحراء الكبرى</text>
  <text x="158" y="80" font-size="12" font-weight="700" fill="#8a6d3b" text-anchor="middle">المغرب</text>
  <defs>
...
CARTE_EMARAT = (
  ...
  <path d="{_MR}" fill="{SABLE}" stroke="{TRAIT}" stroke-width="2"/>
  <path d="{_RIVER}" fill="none" stroke="{BLEU}" stroke-width="2.6" stroke-linecap="round"/>
...
def _frise(events, title, marker_id, vb_w=470):
    ...
    parts = [
        ...
        f'<rect x="1" y="1" width="{vb_w - 2}" height="166" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>',
...
_kh_parts = [
    ...
    f'<defs><marker id="arrK" markerWidth="9" markerHeight="9" refX="6" refY="3" orient="auto">'
    f'<path d="M0,0 L8,3 L0,6 Z" fill="{TRAIT}"/></marker></defs>',
    f'<rect x="1" y="1" width="468" height="148" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>',
    f'<text x="235" y="26" font-size="14" font-weight="700" fill="{TRAIT}" text-anchor="middle">الخلفاء الراشدون رضي الله عنهم</text>',
]
_kh_colors = [VERT, AMBRE, BLEU, ROUGE]
for _i, (_nom, _dates) in enumerate(_khoulafa):
    _bx = 358 - _i * 106  # de droite à gauche
    _c = _kh_colors[_i]
    _kh_parts.append(
        f'<rect x="{_bx}" y="48" width="100" height="62" rx="10" fill="#ffffff" '
        f'stroke="{_c}" stroke-width="2"/>'
        f'<text x="{_bx + 50}" y="74" font-size="12" font-weight="700" fill="{TRAIT}" text-anchor="middle">{_nom}</text>'
        f'<text x="{_bx + 50}" y="96" font-size="12" font-weight="600" fill="{_c}" text-anchor="middle">{_dates}</text>'
        f'<circle cx="{_bx + 50}" cy="128" r="10" fill="{_c}" opacity="0.9"/>'
        f'<text x="{_bx + 50}" y="132.5" font-size="11" font-weight="700" fill="#ffffff" text-anchor="middle">{_i + 1}</text>'
    )
    if _i < 3:
        _kh_parts.append(
            f'<line x1="{_bx - 1}" y1="79" x2="{_bx - 11}" y2="79" '
            f'stroke="{TRAIT}" stroke-width="2" marker-end="url(#arrK)"/>'
        )
...
SCHEMA_INSTITUTIONS = (
  ...
  <rect x="1" y="1" width="418" height="328" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>
...
_rose_parts = [
    _svg_open("0 0 320 320"),
    f'<circle cx="160" cy="160" r="118" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.6"/>',
    f'<circle cx="160" cy="160" r="94" fill="none" stroke="{TRAIT}" stroke-width="1.2" stroke-dasharray="4 4"/>',
]
# pointes intermédiaires (NE, SE, SW, NW) — plus courtes, ambre pâle
...
# pointes principales (N en rouge, E, S, W en sable)
...
_rose_parts.append(
    ...
    f'<text x="160" y="40" font-size="16" font-weight="700" fill="{ROUGE}" text-anchor="middle">شمال</text>'
    f'<text x="160" y="298" font-size="15" font-weight="700" fill="{TRAIT}" text-anchor="middle">جنوب</text>'
    f'<text x="284" y="165" font-size="15" font-weight="700" fill="{TRAIT}" text-anchor="start">شرق</text>'
    f'<text x="36" y="165" font-size="15" font-weight="700" fill="{TRAIT}" text-anchor="end">غرب</text>'
    f'<text x="244" y="72" font-size="11" font-weight="600" fill="#64748b" text-anchor="middle">شمال شرقي</text>'
    f'<text x="76" y="72" font-size="11" font-weight="600" fill="#64748b" text-anchor="middle">شمال غربي</text>'
    f'<text x="244" y="256" font-size="11" font-weight="600" fill="#64748b" text-anchor="middle">جنوب شرقي</text>'
    f'<text x="76" y="256" font-size="11" font-weight="600" fill="#64748b" text-anchor="middle">جنوب غربي</text>'
    "</svg>"
)
...
CLIMAT_ZONES = (
  ...
  <path d="{_MR}" fill="none" stroke="{TRAIT}" stroke-width="2"/>
  <path d="{_RIVER}" fill="none" stroke="{BLEU}" stroke-width="2.6" stroke-linecap="round"/>
  <text x="210" y="26" font-size="14" font-weight="700" fill="{TRAIT}" text-anchor="middle">المناطق المناخية في موريتانيا</text>
  <text x="264" y="120" font-size="12" font-weight="700" fill="#8a6d3b" text-anchor="middle">المنطقة الصحراوية ☀️</text>
  <text x="97" y="232" font-size="11" font-weight="700" fill="{BLEU}" text-anchor="middle"
        transform="rotate(-75 97 232)">المنطقة الساحلية</text>
  <text x="250" y="270" font-size="12" font-weight="700" fill="#1c6b40" text-anchor="middle">منطقة الساحل 🌿</text>
  <!-- légende -->
  <rect x="284" y="296" width="130" height="58" rx="8" fill="#ffffff" stroke="{TRAIT}" stroke-width="1.2"/>
  <rect x="396" y="303" width="12" height="10" fill="{SABLE}" stroke="{TRAIT}" stroke-width="0.8"/>
  <text x="392" y="312" font-size="11" fill="{TRAIT}" text-anchor="start">صحراوي: حار وجاف</text>
  <rect x="396" y="320" width="12" height="10" fill="{OCEAN}" stroke="{TRAIT}" stroke-width="0.8"/>
  <text x="392" y="329" font-size="11" fill="{TRAIT}" text-anchor="start">ساحلي: معتدل</text>
  <rect x="396" y="337" width="12" height="10" fill="{VERT}" opacity="0.4" stroke="{TRAIT}" stroke-width="0.8"/>
  <text x="392" y="346" font-size="11" fill="{TRAIT}" text-anchor="start">ساحل: أمطار صيفية</text>
</svg>'''
)
...
HIERARCHIE_LOIS = (
    ...
    + f'''
  <rect x="1" y="1" width="418" height="238" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>
...
_hq_parts = [
    ...
    f'<rect x="1" y="1" width="418" height="198" rx="12" fill="#fdfaf2" stroke="{VERT}" stroke-width="2"/>',
    f'<rect x="1" y="1" width="418" height="34" rx="12" fill="{VERT}" opacity="0.85"/>',
...
MIZAN_ADL = (
    ...
    + f'''
  <rect x="1" y="1" width="418" height="208" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>
...
SCHEMA_TAAWNIYA = (
  ...
  <defs><marker id="arrT" markerWidth="9" markerHeight="9" refX="6" refY="3" orient="auto">
    <path d="M0,0 L8,3 L0,6 Z" fill="{TRAIT}"/></marker></defs>
  <rect x="1" y="1" width="418" height="248" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>
  <text x="210" y="26" font-size="14" font-weight="700" fill="{TRAIT}" text-anchor="middle">تنظيم التعاونية المدرسية 🤝</text>
  <rect x="130" y="40" width="160" height="38" rx="10" fill="{VERT}" opacity="0.2" stroke="{VERT}" stroke-width="2"/>
  <text x="210" y="64" font-size="12.5" font-weight="700" fill="#1c6b40" text-anchor="middle">الجمعية العامة (كل التلاميذ)</text>
  <path d="M 210,80 L 210,98" fill="none" stroke="{TRAIT}" stroke-width="2" marker-end="url(#arrT)"/>
  <text x="222" y="94" font-size="10" font-weight="600" fill="{TRAIT}" text-anchor="start">تَنتخبُ</text>
  <rect x="120" y="102" width="180" height="38" rx="10" fill="{BLEU}" opacity="0.15" stroke="{BLEU}" stroke-width="2"/>
  <text x="210" y="120" font-size="12.5" font-weight="700" fill="{BLEU}" text-anchor="middle">المكتب التنفيذي</text>
  <text x="210" y="134" font-size="10" font-weight="600" fill="{BLEU}" text-anchor="middle">الرئيس · الأمين العام · أمين المال</text>
  <path d="M 210,142 L 210,160" fill="none" stroke="{TRAIT}" stroke-width="2" marker-end="url(#arrT)"/>
  <text x="222" y="156" font-size="10" font-weight="600" fill="{TRAIT}" text-anchor="start">يُنسّقُ</text>'''
    + "".join(
        f'<rect x="{x}" y="164" width="88" height="52" rx="10" fill="#ffffff" stroke="{AMBRE}" stroke-width="1.8"/>'
        f'<text x="{x + 44}" y="186" font-size="15" text-anchor="middle">{ic}</text>'
        f'<text x="{x + 44}" y="206" font-size="10.5" font-weight="700" fill="#9a5b06" text-anchor="middle">لجنة {lbl}</text>'
        for x, ic, lbl in ((318, "🧹", "النظافة"), (216, "📖", "الثقافة"), (114, "⚽", "الرياضة"), (12, "🌱", "البستنة"))
    )
    + f'''
  <text x="210" y="240" font-size="10" font-weight="600" fill="{TRAIT}" text-anchor="middle">التعاونية هيئة غير حكومية تعلّمنا العملَ الجماعي وتحمّلَ المسؤولية</text>
</svg>'''
)
...
_ORGS = [
    ...
    ("الوحدة الإفريقية", "عضو مؤسس 1963", AMBRE, 296, 208),
    ("اتحاد المغرب العربي", "تأسس 1989", ROUGE, 124, 208),
    ("منظمة استثمار نهر السنغال", "مع السنغال ومالي", "#0e7490", 80, 120),
]
_mn_parts = [
    _svg_open("0 0 420 260"),
    f'<rect x="1" y="1" width="418" height="258" rx="10" fill="#fdfaf2" stroke="{TRAIT}" stroke-width="1.2"/>',
...
for _nom, _sub, _col, _cx, _cy in _ORGS:
    _w = 128 if len(_nom) > 16 else 108
    _mn_parts.append(
...
_PLANI_LABELS = [
    # (x, y, texte, couleur, taille)
    (330, 82, "آسيا", "#7c4a03", 13),
    (250, 145, "إفريقيا", "#7c4a03", 13),
    (243, 52, "أوروبا", "#7c4a03", 11),
    (85, 75, "أمريكا الشمالية", "#7c4a03", 11),
    (152, 165, "أمريكا الجنوبية", "#7c4a03", 10),
    (398, 172, "أستراليا", "#7c4a03", 11),
    (235, 222, "القارة القطبية الجنوبية", "#475569", 9),
...
_plani_parts = [
    _svg_open("0 0 470 240"),
    f'<rect x="1" y="1" width="468" height="238" rx="10" fill="#dbeefe" stroke="{TRAIT}" stroke-width="1.2"/>',
    f'<path d="{_WORLD_LAND}" fill="#f0e2bd" stroke="#64748b" stroke-width="0.7"/>',
    # موريتانيا mise en évidence
    f'<circle cx="222" cy="99" r="5" fill="{VERT}" stroke="#ffffff" stroke-width="1.4"/>',
    f'<text x="214" y="92" font-size="9.5" font-weight="700" fill="#1c6b40" text-anchor="start">موريتانيا</text>',
]
for _x, _y, _t, _c, _fs in _PLANI_LABELS:
    _plani_parts.append(f'<text x="{_x}" y="{_y}" font-size="{_fs}" font-weight="700" fill="{_c}" text-anchor="middle">{_t}</text>')
